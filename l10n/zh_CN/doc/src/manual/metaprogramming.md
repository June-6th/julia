...
The parser yields `QuoteNode`s for simple quoted items like symbols: ```jldoctest interp1 julia> dump(Meta.parse(":x")) QuoteNode value: Symbol x

    <br />`QuoteNode` can also be used for certain advanced metaprogramming tasks.
...
4. Generated functions are only permitted to call functions that were defined *before* the definition of the generated function. (Failure to follow this my result on getting `MethodErrors` referring to functions from a future world-age.)
